...
The public index currently publishes 12 package IDs and 29 versions after
...
The `liltoon-material-override` package has the latest unified `0.2.5` release
containing the Preset Applicator sources, so new projects should install that
one package. Version `0.2.5` also clarifies the single Material Override entry,
makes Preset optional in the creation window, and explains the two Probe Anchor
paths (Build-only or Scene apply) before Scene Preview checks the lilToon package automatically.
...
release is `com.suzuki-y0.jeni.liltoon-material-override@0.2.5`. Eye Texture Adapter
...
`com.suzuki-y0.jeni.liltoon-material-override@0.2.5` is the canonical unified
