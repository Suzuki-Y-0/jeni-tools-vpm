...
`liltoon-material-override` package also has a `0.2.0` release that contains
the Preset Applicator sources, so new projects should install that one package
instead of installing the two lilToon packages side by side. The separate
`liltoon-preset-applicator` `0.1.0` entry remains available for projects that
still depend on the legacy package ID.
...
`com.suzuki-y0.jeni.liltoon-material-override@0.2.0` is the canonical unified
...
project that already has package 9 `0.2.0`; the old `0.1.0` package remains only
for compatibility with existing manifests.
